...
int main(int argc, char* argv[])
{
    ...
    if (argc > 2)
    {
        ...
        /* Ahora transmitimos la información desde los otros procesos al master */
        for (int i = 0; i < totalSize; i++)
        {
            MPI_Status status;
            // El proceso 0 debe ejecutarse en todas las imágenes para recibir la información
            // Los demás procesos se saltan esta ejecución cuando no sea su turno
            ...
            if (myid == 0) {
                ...
                // Si no tiene la información, traigala desde el proceso correspondiente
                if (i % numprocs != 0) {
                    features[i] = (float*)malloc(sizeof(float) * FEATURE_VECTOR_SIZE);
        ...
        if (myid == 0) {
            ...
            // cout << "Proceso " << myid << ", hasta aquí todo bien" << endl;
        }
        return MPI_Finalize();
    }
