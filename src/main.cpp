...
{
    const char* dirname = argc > 1 ? argv[1] : "/root/sistemas_distribuidos/dataset/dataset1/llantas/";
    unsigned int index = 0, i, j, k, l, m, tempI, tempJ;
    string* imagePaths = getImages(dirname);
    Mat img, resizedImg, gx, gy, mag, angle;
    Mat magnitudeValues, angleValues, histograms, output;
    float histogram[8][8][9], Cj[10], vJ, vJp1, singleAngle, singleMagnitude;
    float featureVector[7][7][9];
    ...
    while (imagePaths[index] != "") {
        ...
        img = imread(string(dirname) + imagePaths[index++], IMREAD_GRAYSCALE);
